...
impl PrecompileSetCall {
    ...
    pub fn execute(
        ...
        let output = match self {
            Self::allowance { owner, spender } => {
                let output = <PrecompileSet>::allowance(
                    discriminant,
                    handle,
                    owner,
                    spender,
            ...
            Self::approve { spender, value } => {
                let output = <PrecompileSet>::approve(
                    discriminant,
                    handle,
                    spender,
                    value,
            ...
            Self::balance_of { who } => {
                let output = <PrecompileSet>::balance_of(discriminant, handle, who);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::burn { from, value } => {
                let output = <PrecompileSet>::burn(discriminant, handle, from, value);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::clear_metadata {} => {
                let output = <PrecompileSet>::clear_metadata(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::decimals {} => {
                let output = <PrecompileSet>::decimals(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::eip2612_domain_separator {} => {
                let output = <PrecompileSet>::eip2612_domain_separator(
                    discriminant,
                    handle,
                );
                ::precompile_utils::solidity::encode_return_value(output?)
            ...
            Self::eip2612_nonces { owner } => {
                let output = <PrecompileSet>::eip2612_nonces(
                    discriminant,
                    handle,
                    owner,
                );
            ...
            Self::eip2612_permit { owner, spender, value, deadline, v, r, s } => {
                let output = <PrecompileSet>::eip2612_permit(
                    discriminant,
                    handle,
                    owner,
                    spender,
            ...
            Self::freeze { account } => {
                let output = <PrecompileSet>::freeze(discriminant, handle, account);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::freeze_asset {} => {
                let output = <PrecompileSet>::freeze_asset(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::mint { to, value } => {
                let output = <PrecompileSet>::mint(discriminant, handle, to, value);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::name {} => {
                let output = <PrecompileSet>::name(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::set_metadata { name, symbol, decimals } => {
                let output = <PrecompileSet>::set_metadata(
                    discriminant,
                    handle,
                    name,
                    symbol,
            ...
            Self::set_team { issuer, admin, freezer } => {
                let output = <PrecompileSet>::set_team(
                    discriminant,
                    handle,
                    issuer,
                    admin,
            ...
            Self::symbol {} => {
                let output = <PrecompileSet>::symbol(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::thaw { account } => {
                let output = <PrecompileSet>::thaw(discriminant, handle, account);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::thaw_asset {} => {
                let output = <PrecompileSet>::thaw_asset(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::total_supply {} => {
                let output = <PrecompileSet>::total_supply(discriminant, handle);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::transfer { to, value } => {
                let output = <PrecompileSet>::transfer(discriminant, handle, to, value);
                ::precompile_utils::solidity::encode_return_value(output?)
            }
            Self::transfer_from { from, to, value } => {
                let output = <PrecompileSet>::transfer_from(
                    discriminant,
                    handle,
                    from,
                    to,
            ...
            Self::transfer_ownership { owner } => {
                let output = <PrecompileSet>::transfer_ownership(
                    discriminant,
                    handle,
                    owner,
                );
    ...
    pub fn selectors() -> &'static [u32] {
        &[
            117300739u32, 157198259u32, 404098525u32, 484305945u32, 599290589u32,
            826074471u32, 910484757u32, 936559348u32, 1086394137u32, 1374431959u32,
            1587675670u32, 1804030401u32, 1889567281u32, 2127478272u32, 2367676207u32,
            2514000705u32, 2646777772u32, 2835717307u32, 3352902745u32, 3552201630u32,
            3566436177u32, 3573918927u32, 3714247998u32, 3999121892u32, 4021736498u32,
            4030008324u32, 4076725131u32, 4173303445u32,
        ]
    }
...
impl From<PrecompileSetCall> for ::precompile_utils::__alloc::vec::Vec<u8> {
    fn from(a: PrecompileSetCall) -> ::precompile_utils::__alloc::vec::Vec<u8> {
        a.encode()
    }
}
impl ::fp_evm::PrecompileSet for PrecompileSet {
    fn execute(
        &self,
        handle: &mut impl PrecompileHandle,
    ) -> Option<::precompile_utils::EvmResult<::fp_evm::PrecompileOutput>> {
        use ::precompile_utils::precompile_set::DiscriminantResult;
        let discriminant = <PrecompileSet>::discriminant(
            handle.code_address(),
        ...
        let discriminant = match discriminant {
            ...
            DiscriminantResult::OutOfGas => {
                return Some(Err(::fp_evm::ExitError::OutOfGas.into()));
            }
        };
        Some(
            <PrecompileSetCall>::parse_call_data(handle)
                .and_then(|call| call.execute(discriminant, handle)),
        )
    }
    fn is_precompile(&self, address: H160, gas: u64) -> ::fp_evm::IsPrecompileResult {
        <PrecompileSet>::discriminant(address, gas).into()
    }
}
#[allow(non_snake_case)]
pub(crate) fn __PrecompileSet_test_solidity_signatures_inner() {
